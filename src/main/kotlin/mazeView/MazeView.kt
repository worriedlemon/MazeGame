package mazeGame.mazeView

import mazeGame.mazeControl.MazeControl
...
class MazeView(jar : Boolean = false) : JFrame("Maze Game"), ModelChangeListener {
    class Textures(
        val wall: Icon,
        val emptySpace: Icon,
        val character: Icon,
    ...
    private fun loadSprites(jar : Boolean): Textures {
        val getImage = if (!jar) {
            { path: String -> ImageIcon("src/main/resources/${path}").image }
        } else {
            { path: String -> Toolkit.getDefaultToolkit().getImage(this::class.java.getResource("/${path}")) }
        }
        return Textures(
            ImageIcon(
                getImage("textures/bricks.png").getScaledInstance(
                    TILE_SIZE,
                    TILE_SIZE,
                    SCALE_SMOOTH
                )
            ...
            ImageIcon(
                getImage("textures/grass.png").getScaledInstance(
                    TILE_SIZE,
                    TILE_SIZE,
                    SCALE_SMOOTH
                )
            ...
            ImageIcon(
                getImage("textures/steve.png").getScaledInstance(
                    TILE_SIZE,
                    TILE_SIZE,
                    SCALE_SMOOTH
                )
            ...
            ImageIcon(
                getImage("textures/finish.png").getScaledInstance(
                    TILE_SIZE,
                    TILE_SIZE,
                    SCALE_SMOOTH
                )
    ...
    private val textures = loadSprites(jar)
